...
def _write_modules_gitignore(app_dir: Path, nsx_cfg: dict[str, Any]) -> None:
    ...
    for item in modules:
        ...
        if source and source.get("vendored") is True:
...
def _normalize_module_source(cfg: dict[str, Any]) -> None:
    ...
    for item in modules:
        ...
        if isinstance(path, str) and path:
            item["local"] = True
            entry = mod_overrides.setdefault(name, {})
            ...
            proj_overrides.setdefault(name, {"local_path": path})
