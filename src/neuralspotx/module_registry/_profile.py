...
def expand_profile_seeds(
    ...
    * ``baseline: none`` opts out of profile seeding entirely, making the
    ...
    ``module_registry`` block is preserved. Synthetic profile registry defaults
    are kept separately so effective resolution can apply them below explicit
    workspace/inline layers and authored app-local overrides.
    """
    ...
    expanded = dict(nsx_cfg)
    expanded["modules"] = modules
    expanded["_profile_registry"] = module_registry
    return expanded
