...
def lock_path(app_dir: Path, board: str | None = None) -> Path:
    """Path to the app's lock file.

    Single-target apps use the legacy unsuffixed ``nsx.lock`` (*board* is
    ``None``); multi-target apps key the lock per board as
    ``nsx.<board>.lock`` so each committed target is reproducible.
    """

    if board:
        return app_dir / f"nsx.{board}.lock"
    return app_dir / LOCK_FILENAME


def read_lock(app_dir: Path, board: str | None = None) -> NsxLock | None:
    """Read the lock from *app_dir*, or return ``None`` if missing.
...
def write_lock(app_dir: Path, lock: NsxLock, board: str | None = None) -> Path:
    """Write *lock* to the app's lock file atomically and return the path.

    Writes through a same-directory temp file + ``os.replace`` so a
    crash or Ctrl-C mid-write cannot leave a half-written lock file
