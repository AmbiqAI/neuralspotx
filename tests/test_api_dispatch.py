...
def test_build_and_flash_dispatch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    flash_calls: list[tuple[Path, str | None, Path | None, str | None, str | None, int, Path | None, bool]] = []
    ...
    def fake_flash(
        ...
        *,
        ...
        flash_calls.append((app_dir, board, build_dir, toolchain, probe_serial, jobs, sdk_root, frozen))
    ...
    assert flash_calls == [
        (app_dir.resolve(), None, build_dir.resolve(), None, "1160002204", 2, None, True)
    ]
